...
function parseLog(result:string) {
    ...
    let train_loss = result.match(/I\d{4}\s\d{2}:\d{2}:\d{2}\.\d{6}\s+\d+\ssolver.cpp:\d+]\s+Train net output #0: loss = \d+(\.\d+)?([eE][+-]\d+)?/g)
                           .map((str) => {return str.match(/loss = \d+(\.\d+)?([eE][+-]\d+)?/)[0];})
                           .map((str) => {return str.match(/\d+(\.\d+)?([eE][+-]\d+)?/)[0];})
                           .map((str) => {return Number(str);})
    let test_loss = result.match(/I\d{4}\s\d{2}:\d{2}:\d{2}\.\d{6}\s+\d+\ssolver.cpp:\d+]\s+Test net output #1: loss = \d+(\.\d+)?([eE][+-]\d+)?/g)
                          .map((str) => {return str.match(/loss = \d+(\.\d+)?([eE][+-]\d+)?/)[0];})
                          .map((str) => {return str.match(/\d+(\.\d+)?([eE][+-]\d+)?/)[0];})
                          .map((str) => {return Number(str);})
